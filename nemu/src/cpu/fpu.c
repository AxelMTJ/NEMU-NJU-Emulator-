...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)
	{
			   ...
			   )
		{
			...
			bool stick=((sig_grs&(0x3))==0)?0:1;
			sig_grs>>=1;
			sig_grs=(stick)?sig_grs|0x1:sig_grs&0xfffffffffffffffe;
			exp++;
		}
	...
	if (!overflow)
	{
		...
		//printf("grs=%d, ",grs);
		//if(grs>4 || (grs==4 &&((sig_grs&(0x8))!=0)))
		uint32_t sig_last=sig_grs&(0x8);
		//printf("sig_last=%d, ",sig_last);
		//bool sig_iszero=(sig_grs==0x4000004)?1:0,sigtest2=(sig_grs==0x4000008 || sig_grs==0x4000010)?1:0;
		//printf("sig=%llx, ",sig_grs);
		bool addone=(grs==4 && sig_last!=0)?1:0;
		//if(((grs>4) || addone) || (sig_iszero)||sigtest2)
		if((grs>4) || addone )
		{
			//printf("add one, ");
			sig_grs+=0x8;
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	if(fb.exponent==0)
		fb.exponent++;
	shift=(fa.exponent>fb.exponent)?(fa.exponent-fb.exponent):(fb.exponent-fa.exponent);
